...
public class Board {
    ...
    private Map<kalah.Player, Integer> playerMap = new HashMap<>();
    ...
    //return seeds in house
    public int getHouseSeeds(Player player, int houseNumber){
        return playerPits.get(playerMap.get(player) + houseNumber).getSeeds();
    }

    //return players store seeds
    public int getStoreSeeds(Player player){
        return playerPits.get(playerMap.get(player) + quantityOfHouses).getSeeds();
    }
    ...
    private void playerMove(int houseMove){
        ...
        if (playerPits.get(index).getSeeds() == 0 || houseMove > quantityOfHouses){
